...
export const ELEMENTOS = ["triangulo", "concentricos", "anel", "disco", "arco", "elos", "virgula", "reta", "deco", "ventania", "circulo-triangulo", "seta", "onda", "faixa-quadrado", "paulista"];

/**
 * O elemento "paulista" (fatia 4 do redesenho, 2026-09-20 — `Pagina Cidade -
 * Sao Paulo.dc.html`): a banda em Z da calçada da Paulista, duas dobras a 45°,
 * recebe a cor e o papel fica à mostra. NÃO é motivo do motor Athos (o
 * `athosPatterns.js` segue verbatim): é um clip-path, desenhado aqui como no
 * protótipo. Só entra DECLARADO — a cidade de São Paulo o declara em
 * content/cidades.ts; o sorteio de lib/tweaks.mjs nunca o escolhe (CLAUDE.md
 * do pacote: "o elemento paulista só em páginas de São Paulo").
 */
export const Z_PAULISTA = "polygon(25% 25%,50% 25%,75% 50%,100% 50%,75% 75%,50% 75%,25% 50%,0 50%)";
...
export function heroPecas({ elemento = "triangulo", pano = "diagonal", cores = "cinza · ouro", seed = 5 } = {}, rows = 5) {
  ...
  const athos = elem !== "triangulo" && elem !== "paulista" && byId(elem);
  const n = cols * rows;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (on) {
      const c = r();
      if (elem === "paulista") {
        // O pano decide quais casas se preenchem; cada peça gira 0/90/180/270
        // ao acaso, misturando vertical e horizontal (protótipo, verbatim).
        clip = Z_PAULISTA;
