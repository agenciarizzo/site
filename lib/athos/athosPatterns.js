...
// 17–19 — motivos das fotos (Athos Bulcão): seta escalonada, onda de quartos, faixa com quadradinho
const seta = c => `linear-gradient(${c},${c}) 50% 50%/34% 100% no-repeat, linear-gradient(${c},${c}) 50% 0%/68% 34% no-repeat, linear-gradient(${c},${c}) 50% 34%/100% 33% no-repeat`;
const onda = c => `radial-gradient(100% 100% at 0% 0%, ${c} 0 99%, transparent 99.5%) no-repeat, radial-gradient(100% 100% at 100% 100%, ${c} 0 99%, transparent 99.5%) no-repeat`;
const faixaQ = (c, d) => `linear-gradient(45deg, transparent 0 38%, ${c} 39% 61%, transparent 62%) no-repeat, linear-gradient(${d},${d}) 82% 18%/14% 14% no-repeat`;
...
export const PATTERNS = [
  ...
  { id: 'seta', num: '17', name: 'Seta escalonada', motivo: 'cruz/seta em degraus', maxCores: 2,
    note: 'A seta pixelada do painel amarelo: degraus retos que viram cruz quando as peças se encontram.',
    recipe: ([a, b]) => [{ bg: seta(a), w: 4 }, { bg: seta(b), w: 1 }, { bg: 'transparent', w: 2 }] },
  { id: 'onda', num: '18', name: 'Onda de quartos', motivo: 'dois quartos opostos', maxCores: 2,
    note: 'Dois quartos de círculo em cantos opostos: a peça sozinha é um S; em conjunto vira onda.',
    recipe: ([a, b]) => [{ bg: onda(a), w: 3 }, { bg: onda(b), w: 2 }, { bg: 'transparent', w: 2 }] },
  { id: 'faixa-quadrado', num: '19', name: 'Faixa & quadradinho', motivo: 'diagonal + quadrado solto', maxCores: 2,
    note: 'A faixa diagonal do painel de 1987, pontuada por um quadradinho de cor contrastante.',
    recipe: ([a, b]) => [{ bg: faixaQ(a, b), w: 3 }, { bg: reta(a), w: 1 }, { bg: sq(b, 16, 30, 70), w: 1 }, BLANK] },
];
